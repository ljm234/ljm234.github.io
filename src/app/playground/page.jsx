...
export default function Playground(){
  const [x, setX] = useState({
    freshwater_exposure: 1, days_since_exposure: 4,
    fever: 1, headache: 1, nuchal_rigidity: 0,
    csf_wbc: 200, csf_protein: 80, csf_glucose: 40,
    pcr_available: 0, microscopy_available: 0
  });
      ...
      <section className="space-y-4">
        <div className="rounded-lg border p-4">
          ...
          <div className="text-4xl font-bold">
            {(out.p*100).toFixed(1)}% <span className="text-base font-normal ml-2">({out.band})</span>
          </div>
          <div className="text-xs text-neutral-500 mt-1">
            Uses a simple logistic model for demo; uncertainty not calibrated to real data.
          </div>
        </div>
  ...
  return (
    <label className="flex items-center justify-between gap-2 rounded border px-3 py-2">
      ...
      <button
        ...
        onClick={(e)=>{e.preventDefault(); on(v?0:1);}}
        aria-pressed={v===1}
      >{v? "Yes":"No"}</button>
    </label>
  );
