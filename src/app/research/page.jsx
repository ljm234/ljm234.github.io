...
const items = [
  {
    ...
    desc:
      "Risk model with calibration, decision-curve analysis, and explainability.",
  ...
  },
  {
    ...
    desc:
      "Paired HeLa+Nf vs Nf-only plates across LDH (24 h), Caspase-3 (48 h), and JC-1 (72 h) nominate ER/COPII and ER–mitochondria Ca²⁺ coupling.",
    tags: ["Mechanism", "Screening"],
    preview: {
      abstract:
        "Screens on matched plates highlight organelle vulnerabilities that are selective for Naegleria.",
      figures: [
        ...
        {items.map((p) => (
          <article key={p.slug} className="rounded-2xl border p-6">
            ...
            <div className="mt-3 flex items-center">
              <a className="underline" href={`/research/${p.slug}`}>
                Open project →
              </a>
              {p.preview && (
                <PreviewButton
                  label="Quick Preview"
                  abstract={p.preview.abstract}
                  items={p.preview.figures}
                />
              )}
            </div>
          </article>
