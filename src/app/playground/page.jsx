            ...
            {parts.slice(0,6).map(p=>(
              <li key={p.feature} className="flex justify-between">
                ...
                <span className="tabular-nums">
                  {Number.isFinite(p.contrib) ? p.contrib.toFixed(2) : "—"}
                </span>
              </li>
            ))}
          </ul>
...
function Form({x,setX}){
  return (
    <div className="grid sm:grid-cols-2 gap-4">
      ...
      <NumberInput label="Days since exposure" min={0} max={14} step={1} v={x.days_since_exposure} on={v=>setX({...x, days_since_exposure: v})}/>
      ...
      <NumberInput label="CSF WBC (cells/µL)" min={0} max={5000} step={10} v={x.csf_wbc} on={v=>setX({...x, csf_wbc: v})}/>
      <NumberInput label="CSF Protein (mg/dL)" min={0} max={1000} step={5} v={x.csf_protein} on={v=>setX({...x, csf_protein: v})}/>
      <NumberInput label="CSF Glucose (mg/dL)" min={0} max={200} step={1} v={x.csf_glucose} on={v=>setX({...x, csf_glucose: v})}/>
